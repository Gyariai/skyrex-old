...
import { qrcodesecret, qrcodesecretauth } from "../../../../axios/2fa"
...
export const EnableQR = ({ status, close }) => {
    const [ qrlink, setQr] = useState(false)
    ...
    const Auth = async () => {
        const res = await qrcodesecretauth(email, token, qrlink.base32)
        if(res) {
            setErr(false)
            setAuth(true)
        } else {
            ...
            </div>
            {
                ...
                <div style={{display: "flex", alignItems: "center", flexDirection: "column"}}>
                    ...
                    <span className="title-fa-h">
                        Google Auth connected. Please login now.
                    </span>
                    <br />
                    <div className="dashboard-btn-fa"  onClick={() => window.location.reload()}>
                        <span className="title-fa-h" style={{color: "white"}}>Ok</span>    
                    </div>
                </div>
                    :
                <div style={{display: "flex", alignItems: "center", flexDirection: "column"}}>
                    <span className="title-fa-h">Connect Google authenticaor</span>
                    <span className="title-fa-s">After 2FA activation the IP-adress check will be disabled.</span>
                    <span className="title-fa-s">If necessary, you can enable it yourself.</span>
                    <div style={{
                        padding: 10
                    }}>
                       <QRCodeCanvas value={qrlink.otpauth_url} />
                    </div>
                    <span className="title-fa-s">Don’t forget to back up your secret key:</span>
                    <span className="title-fa-h"></span>
                    ...
                    <Err err={err}/>
                    <div style={{width: "100%", display: "flex", justifyContent: "space-between", paddingTop: 10}} onClick={() => Auth()}>
                        <div className="dashboard-btn-fa">
                            <span className="title-fa-h" style={{color: "white"}}>Ok</span>    
                        </div>
                        <div style={{padding: 5}}/>
                        <div className="dashboard-btn-fa"  style={{backgroundColor: "#3A3A3C"}} onClick={() => close()}>
